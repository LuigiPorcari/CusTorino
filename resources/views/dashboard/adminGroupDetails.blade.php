<x-layout documentTitle="Admin Group Details">
    ...
    <main>
        <h1 class="visually-hidden">Dettagli gruppo amministratore</h1>
                ...
                <div class="col-12 col-md-4 mt-5">
                    ...
                    <div class="card admin-card">
                        <div class="admin-card-body">
                            ...
                            @else
                                <p class="card-text">Primo allenatore: <br> Nessuno</p>
                            @endif
                            ...
                            <div class="d-flex flex-column align-items-center mt-5">
                                ...
                                <div class="text-center w-100">
                                    <a href="{{ route('admin.dashboard') }}"
                                        class="btn admin-btn-info w-100 fs-6 ">Torna
                                        alla
                                        lista
                                        Gruppi</a>
                                </div>
                ...
                <div class="col-12 col-md-8 mt-md-5">
                    ...
                    <div id="archiveSection" class="table-responsive d-none">
                        ...
                        <div class="mb-3">
                            ...
                            <select id="archiveFilter" class="custom-form-input" aria-label="Filtra archivio per data">
                            ...
                            <tbody>
                                @foreach ($otherAliases as $alias)
                                    <tr class="archive-row bg-light" data-date="{{ $alias->data_allenamento }}">
                                        <td
                                            class="@if (is_null($alias->primo_allenatore_id)) bg-danger text-white @elseif($alias->check_conf) bg-warning @endif">
                    ...
                    <div class="mb-3">
                        <label for="filter_data_allenamento" class="form-label">Filtra per data:</label>
                        <select id="filter_data_allenamento" class="custom-form-input"
                            aria-label="Filtra allenamenti per data">
                            <option value="">Tutte le date</option>
                    ...
                    <div class="modal fade" id="deleteModalGroup" tabindex="-1"
                        aria-labelledby="deleteModalGroupLabel" aria-hidden="true">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <div class="modal-header admin-modal-header">
                                    <h1 class="modal-title fs-5" id="deleteModalGroupLabel">Sicuro di voler eliminare
                                        questo
                                        Gruppo?</h1>
                    ...
                    @foreach ($aliases as $alias)
                        <div class="modal fade" id="deleteModalAlias{{ $alias->id }}" tabindex="-1"
                            ...
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-header admin-modal-header">
                                        <h1 class="modal-title fs-5" id="deleteModalAliasLabel{{ $alias->id }}">
                                            Sicuro di voler eliminare il gruppo Alias in data
                                    ...
                                    <div class="modal-body d-flex justify-content-center admin-modal-body">
                                        <form
                                            action="{{ route('alias.delete', $alias->id, $availableDates, $group) }}"
                                            method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn admin-btn-danger mx-2">Si</button>
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body admin-modal-body">
                                    <form action="{{ route('storeAlias', $group->id) }}" method="POST">
                                        @csrf
                                        <div class="mb-3">
                                            <label class="custom-form-label" for="form_data_allenamento">In che data
                                                vuoi creare il gruppo alias?</label>
                                            <select class="custom-form-input" id="form_data_allenamento"
                                                name="data_allenamento" required
                                                aria-label="Data per il nuovo gruppo alias">
                                                @forelse ($dateMancanti as $date)
                                                    <option value="{{ $date }}">
                                                        {{ \Carbon\Carbon::parse($date)->translatedFormat('l d F') }}
                                                    </option>
                                                @empty
                                                    <option value="">Non ci sono date disponibili</option>
                                                @endforelse
                                            </select>
                                        </div>
                                </div>
                    ...
                    <script>
                        ...
                        document.getElementById('archiveFilter').addEventListener('change', function() {
                            ...
                            rows.forEach(row => {
                                row.style.display = (filterValue === '' || row.dataset.date === filterValue) ? '' : 'none';
                            });
                        });

                        document.getElementById('filter_data_allenamento').addEventListener('change', function() {
                            const filterValue = this.value;
