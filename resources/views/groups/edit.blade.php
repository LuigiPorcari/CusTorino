<x-layout documentTitle="Modify Groups">
    ...
    <form method="POST" action="{{route('groups.update' , $group)}}">
        ...
        <div class="form-group">
            ...
            <input value="{{ $group->campo }}" type="number" class="form-control" id="campo" name="campo" min="1" max="4"
                required>
        </div>
        {{-- GENERE --}}
        <div class="form-group">
        ...
        <div class="form-group">
            ...
            <select class="form-control" id="primo_allenatore_id" name="primo_allenatore_id" required>
                @foreach ($trainers as $trainer)
                    <option @if ($group->primo_allenatore_id == $trainer->id) selected @endif value="{{ $trainer->id }}">{{ $trainer->nome }} {{ $trainer->cognome }}</option>
                @endforeach
            </select>
        </div>
        {{-- SECONDO ALLENATORE --}}
        <div class="form-group">
            ...
            <select class="form-control" id="secondo_allenatore_id" name="secondo_allenatore_id">
                ...
                @foreach ($trainers as $trainer)
                    <option @if ($group->secondo_allenatore_id == $trainer->id) selected @endif value="{{ $trainer->id }}">{{ $trainer->nome }} {{ $trainer->cognome }}</option>
                @endforeach
            </select>
        </div>
        {{-- CONDIVISO --}}
        ...
        <div class="form-group">
            ...
            <input value="{{$group->livello}}" type="number" class="form-control" id="livello" name="livello" required min="1"
                max="10">
        </div>
        {{-- STUDENTI --}}
        <div class="form-group">
            ...
            @foreach ($students as $student)
                <div class="form-check">
                    <input @if ($group->students->contains($student)) checked @endif class="form-check-input" type="checkbox" value="{{ $student->id }}" id="radioStudenti"
                        name="studenti_id[]">
