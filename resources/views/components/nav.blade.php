<nav class="navbar navbar-expand-lg custom-navbar fixed-top shadow-sm ps-1" id="neubar">
    <div class="container-fluid">
        ...
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
                        ...
                        <ul class="dropdown-menu custom-dropdown-menu">
                            @if (Auth::user()->is_admin)
                                <li class="d-flex justify-content-center">
                                    <a class="btn custom-btn-primary mb-2" href="{{ route('groups.create') }}">Crea
                                        gruppi</a>
                                </li>
                            @endif
                            <li class="d-flex justify-content-center">
                                <a class="btn custom-btn-primary mb-2" href="{{ route('password.change') }}">Cambia
                                    Password</a>
                            </li>
                            <li class="d-flex justify-content-center">
                                <form action="{{ route('logout') }}" method="POST">
                                    @csrf
                                    <button type="submit" class="btn custom-btn-warning mb-2">Logout</button>
                                </form>
                            </li>
                            @if (Auth::user()->is_admin)
                                <li class="d-flex justify-content-center">
                                    <button type="button" class="btn custom-btn-danger" data-bs-toggle="modal"
            ...
            @if (!Auth::check())
                <div>
                    <a class="btn custom-btn-primary" href="{{ route('login') }}">Accedi</a>
                    <a class="btn custom-btn-primary mx-2" href="{{ route('corsista.register') }}">Registrati come
                        Corsista</a>
                </div>
            @endif
            @if (Auth::check() && Auth::user()->is_admin)
                <div class="d-flex">
                    <a class="btn custom-btn-primary me-2" href="{{ route('corsista.register') }}">Registra nuovo
                        Corsista</a>
                    <a class="btn custom-btn-primary me-2" href="{{ route('trainer.register') }}">Registra nuovo
                        Trainer</a>
                    <a class="btn custom-btn-primary" href="{{ route('admin.register') }}">Registra nuovo Admin</a>
                </div>
            @endif
        </div>
    </div>
